# -*- coding: utf-8 -*-
"""
Created on Thu Jun  8 00:23:52 2023
...
def build_model(NUM_TIMESTEPS, NUM_INPUT_FEATS, NUM_OUTPUT_FEATS, MASK_VALUE,
                transformer_units = 512, num_heads = 8, dropout_rate = 0.4, dense_act = 'sigmoid', learning_rate = 0.001, loss = 'binary_crossentropy', metrics=['binary_accuracy']):
    ...
    # Transformer layer
    model_layer = keras.layers.MultiHeadAttention(num_heads=num_heads, key_dim=transformer_units)(model_layer, model_layer)
    model_layer = keras.layers.Dropout(dropout_rate)(model_layer)
    model_layer = keras.layers.LayerNormalization(epsilon=1e-6)(model_layer)
    model_layer = keras.layers.Dense(units=transformer_units, activation='relu')(model_layer)
    model_layer = keras.layers.Flatten()(model_layer)
    model_layer = keras.layers.Dense(units=NUM_TIMESTEPS * NUM_INPUT_FEATS)(model_layer)
    model_layer = keras.layers.Reshape((NUM_TIMESTEPS, -1))(model_layer)
    
    model_outputs = keras.layers.Dense(NUM_OUTPUT_FEATS, activation=dense_act)(model_layer)  # sigmoid for classification

    # Create model object and compile the model:
    model_out = keras.Model(inputs=model_inputs, outputs=model_outputs, name='Transformer')
