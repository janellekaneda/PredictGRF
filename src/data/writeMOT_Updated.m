function [] = writeMOT_Updated(data,header,filePath,fileName,version,inDegrees) ;
% Function updated by Janelle K (Nov 2022): updated header entries to be
% consistent with OAGR file types

% Columnwise data (time 1st col), header in a cell
% version: version number (int)
% inDegrees: 1 = yes, 0 = no

    fid = fopen([filePath '\' fileName '.mot'],'w') ;
% Write the header
    fprintf(fid,'%s\n',[fileName '.mot']);
    ...
    disp(['Successfully wrote ' filePath '\' fileName '.mot to file.']) ;