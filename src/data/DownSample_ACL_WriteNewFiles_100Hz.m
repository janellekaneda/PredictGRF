        ...
        if isfile(ik_res_updated_path)
            ...
            if size(ik_data_new,1) ~= size(grf_data_new,1) % if off, usually off by one
                if size(ik_data_new,1) < size(grf_data_new,1)
                    ...
                    grf_data_new_fix = interp1(grf_data_new(:,1), grf_data_new, ik_data_new(:,1));
                else % IK dim bigger than GRF dim, so downsample IK data size
                    ik_data_new_fix = interp1(ik_data_new(:,1), ik_data_new, grf_data_new(:,1));
                end
...
writecell(trials_diff_ds_size, 'DownSample_ACL_trials_diff_ds_size_FirstHalf_FixOutputSize_100Hz.xlsx');
writecell(trials_notupdated, 'DownSample_ACL_trials_notupdated_FirstHalf_FixOutputSize_100Hz.xlsx');
