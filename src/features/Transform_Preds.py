...
MAX_TIMESTEPS = 67
MASK_VALUE = 999
NUM_OUTPUT_FEATS = 12 # back into 3D vec form
...
Y_true = np.load(os.path.join(BASE_DIR, 'data', 'processed', 'normalized', exp_type, 'Y_test_norm.npy'), allow_pickle=True) # shape: (trials, max timesteps, 16)
Y_pred = np.load(os.path.join(BASE_DIR, 'models', exp_type, 'testset_predictions.npy'), allow_pickle=True)
...
Y_train_means = np.load(os.path.join(BASE_DIR, 'data', 'processed', 'normalized', exp_type, 'Y_train_means.npy'), allow_pickle=True)
Y_train_stds = np.load(os.path.join(BASE_DIR, 'data', 'processed', 'normalized', exp_type, 'Y_train_stds.npy'), allow_pickle=True)
...
for subj in test_subjs_oagr:
    ...
    for cond in oagr_conds:
        ...
        for step in steps_list:
            ...
            if use_transforms:
                ...
                ikrespath = os.path.join(BASE_DIR, 'data', 'raw', 'OAGR_DownSampledFiles', ('Subject_' + subj), cond, ('results_ik_step' + step + '_Fs60.sto'))
    ...
    # =============================================================================
                ...
                Y_true_transformed[test_ix_all,0:min_timestep,:] = np.expand_dims(np.hstack([true_grf_r_t, true_grf_l_t, true_grm_r_t, true_grm_l_t]), axis=0)
                Y_pred_transformed[test_ix_all,0:min_timestep,:] = np.expand_dims(np.hstack([pred_grf_r_t, pred_grf_l_t, pred_grm_r_t, pred_grm_l_t]), axis=0)
                ...
                # Place into big transformed datastructs (contains masked value)!
                Y_true_transformed[test_ix_all,0:min_timestep,:] = np.expand_dims(np.hstack([true_grf_r_3d, true_grf_l_3d, true_grm_r_3d, true_grm_l_3d]), axis=0)
                Y_pred_transformed[test_ix_all,0:min_timestep,:] = np.expand_dims(np.hstack([pred_grf_r_3d, pred_grf_l_3d, pred_grm_r_3d, pred_grm_l_3d]), axis=0)
...
for subj in test_subjs_acl:
    ...
    split1 = [feat.split(f"_{subj}_")[1] for feat in feats_list]
...
outdir = os.path.join(BASE_DIR, 'models', exp_type, 'transformed_preds')
if use_transforms:
    np.save(os.path.join(outdir, 'Y_true_transformed.npy'), Y_true_transformed)
    np.save(os.path.join(outdir, 'Y_pred_transformed.npy'), Y_pred_transformed)
else:
    np.save(os.path.join(outdir, 'Y_true_3d.npy'), Y_true_transformed)
    np.save(os.path.join(outdir, 'Y_pred_3d.npy'), Y_pred_transformed)    
