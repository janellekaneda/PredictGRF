...
SEED = 43
WITH_CUTTING = True # whether to put cutting tasks in training set
# # # # # # # # # 
...
if WITH_CUTTING:
    acl_tasks_train = acl_tasks
    NUM_ACL_TRIALS_TRAIN = NUM_ACL_TRIALS_TEST_DEV
...
# Load dict of trials to upsample
trials_to_upsample = np.load(os.path.join(BASE_DIR, "references\ACL_TrialsToUpsample.npz"), allow_pickle=True)
...
for subj in subjects_oagr:
    ...
    for cond in oagr_conds:
        ...
        feats_list = [x for x in feats_list if '100Hz_RawDist' in x]
...
for subj in subjects_acl:
    ...
    if subj in train_subjs_acl: # only use feats_list_lc_train
        ...
        if len(feats_list_lc_train) == NUM_ACL_TRIALS_TRAIN: # append normally             
            ...
            for task in acl_tasks_train:
                ...
                if task_count == 2: # randomly select one of the existing two trials, and upsample that
                    ...
                    # Then, add the trial from the upsample dict.
                    
                    # Load feature matrix.
                    feats = np.load(os.path.join(BASE_DIR, 'data', 'interim', 'ACL_FeatureMatrices', subj, str(trials_to_upsample[subj + task])), allow_pickle=True)
        ...
        if len(feats_list_lc) == NUM_ACL_TRIALS_TEST_DEV: # append normally             
            ...
            for feats_filename in feats_list_lc:
            ...
            for task in acl_tasks:
                ...
                if task_count == 2: # randomly select one of the existing two trials, and upsample that
                    ...
                    if subj in test_subjs_acl:
                        print(f"UPSAMPLE: Appending to test set, {subj}, {str(trials_to_upsample[subj + task])}, ix: {test_ix_all}")
                        test_set_trials.append(f"Appending to test set, {subj}, {str(trials_to_upsample[subj + task])}, ix: {test_ix_all}")
...
if WITH_CUTTING:
    ...
    np.save(os.path.join(outpath, 'Y_train_cutting_100Hz_RawDist.npy'), Y_train)
else:
    np.save(os.path.join(outpath, 'X_train_100Hz_RawDist.npy'), X_train)
    np.save(os.path.join(outpath, 'Y_train_100Hz_RawDist.npy'), Y_train)
np.save(os.path.join(outpath, 'X_dev_100Hz_RawDist.npy'), X_dev)
np.save(os.path.join(outpath, 'Y_dev_100Hz_RawDist.npy'), Y_dev)
np.save(os.path.join(outpath, 'X_test_100Hz_RawDist.npy'), X_test)
np.save(os.path.join(outpath, 'Y_test_100Hz_RawDist.npy'), Y_test)

# Write test set trials to file.
with open(os.path.join(BASE_DIR, 'references', 'test_set_trials_100Hz_RawDist.txt'), 'w') as f:
    for entry in test_set_trials:
        f.write(entry + '\n')

# Check that appended correctly...
assert train_ix_all == num_train_ex_oagr + num_train_ex_acl, f"ERROR: train indices don't match total number of training examples. train_ix_all: {train_ix_all}; num training ex: {num_train_ex_oagr + num_train_ex_acl}"
assert dev_ix_all == num_dev_ex_oagr + num_dev_ex_acl, "ERROR: dev indices don't match total number of training examples. dev_ix_all: {dev_ix_all}; num dev ex: {num_dev_ex_oagr + num_dev_ex_acl}"
assert test_ix_all == num_test_ex_oagr + num_test_ex_acl, "ERROR: test indices don't match total number of training examples. test_ix_all: {test_ix_all}; num test ex: {num_test_ex_oagr + num_test_ex_acl}"
